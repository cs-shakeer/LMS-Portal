...
<%@ page import="com.LMS_Java.model.MD_LC_Request"%>
<!DOCTYPE html>
<html>

<head>
<meta charset="UTF-8">
<title>Lab Coordinator Dashboard</title>
<style>
* {
	margin: 0;
	padding: 0;
...
<body>
    ...
    <form action="LabCord_Logout" method="post" name="labCord_logout">
        <button type="submit" class="btn" onclick="return confirm('Are you sure?')">Logout</button>
				 ...
				 <div id="userProfile">
				<% List<MD_LC_Request> currentLabCordDetails = (List<MD_LC_Request>) 
				                                          request.getAttribute("currentLabCordDetails");
												if (currentLabCordDetails != null) {
												for (MD_LC_Request ct : currentLabCordDetails) {
												%>
				 <p>Id : <%=ct.getLcId()%></p>
				<p>Name : <%=ct.getLcName()%></p>
				<p>Email : <%=ct.getLcEmail()%></p>
				<p>Mobile : <%=ct.getLcMobile()%></p>
				<p>password : <%=ct.getLcPsw()%></p>
				<p>Course Id : <%=ct.getCrId()%></p> 
				
				<a class="btn" href="#section5"
				onclick="profileEdit('<%=ct.getLcId()%>', '<%=ct.getLcName()%>', '<%=ct.getLcEmail()%>', '<%=ct.getLcMobile()%>', '<%=ct.getLcPsw()%>','<%=ct.getCrId()%>')">Profile edit</a>
		...
		<div class="rightSide-container">
			<div class="rightTop">
				<h3>
					Welcome,
					<%= session.getAttribute("labCordName") %>
					👋
				</h3>
            </div>
			...
			<div class="rightBottom">
				...
				<h1>All Student Count : <%=request.getAttribute("studentCount") %></h1>
					<h2 style="text-align: center;">Lab Coordinator Dashboard</h2>
					...
					<h2 style="text-align: center;">Batch List</h2>
					 <table border="1"
						...
						<% List<MD_LC_Request> batchList = (List<MD_LC_Request>)
												request.getAttribute("batchList");
												if (batchList != null) {
												for (MD_LC_Request b : batchList) {
												%>
						<tr>
							<td><%=b.getBname()%></td>
							<td><%=b.getCrId()%></td>
							<td><%=b.getBatchCount()%></td>
				...
				<div class="set-Form" id="section2">
					<h2 style="text-align: center;">Students List</h2>
					 <table border="1"
						...
						<% List<MD_LC_Request> studentList = (List<MD_LC_Request>)
												request.getAttribute("studentList");
												if (studentList != null) {
												for (MD_LC_Request s : studentList) {
												%>
						<tr>
							<td><%=s.getStId()%></td>
							<td><%=s.getStName()%></td>
				...
				<div>
					<h2 style="text-align: center;">Contents List</h2>
					 <table border="1"
						style="width: 90%; margin: 20px auto; background: white;">
						<tr>
							<th>Title</th>
							...
							<th>CaseStudy</th>
							<th>Batch</th>
							<th>Lab Coordinator</th>
							<th>Course</th>
							<th>Action</th>
						</tr>
						<% List<MD_LC_Request> contentList = (List<MD_LC_Request>)
												request.getAttribute("contentList");
												if (contentList != null) {
												for (MD_LC_Request c : contentList) {
												%>
						<tr>
							<td><%=c.getLctTitle()%></td>
							<td><a href="<%=c.getLctAssig()%>" target="_blank" style="color:black;">Link</a></td>
							<td><a href="<%=c.getLctCaseStudy()%>" target="_blank" style="color:black;">Link</a></td>
							<td><%=c.getBname()%></td>
							<td><%=c.getLcId()%></td>
							<td><%=c.getCrId()%></td>
							<td><form action="LC_ConDelete" method="post"
									style="display: inline;">
									<input type="hidden" name="conTitle" value="<%=c.getLctTitle()%>">
									<input type="hidden" name="conMetiral" value="<%=c.getLctAssig()%>">
									<input type="hidden" name="conAssig" value="<%=c.getLctCaseStudy()%>">
									<input type="hidden" name="stId" value="<%=c.getLcId()%>">
				...
				<h1>Content Upload</h1>
					<form action="LC_ConUpload" method="post" name="lc_conupload"
						class="st-RForm">
						<input type="hidden" id="conlcId" name="lcId"
							value="<%= session.getAttribute("labCordId")%>">
							...
							<input type="text" id="conTitle" name="lctTitle"  placeholder="Title" required>
							<input type="text" id="conMeterial"
								name="lctAssig" placeholder="Assignment Link"> <input
								type="text" id="conAssig" name="lctCaseStudy"
								placeholder="CaseStudy Link">
				...
				<h2 style="text-align: center;">Profile Update</h2>
					<form action="LC_TprofileUpdate" method="post" name="lc_tprofileupdate"
						class="st-RForm">
